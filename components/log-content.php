<?php 
namespace wc_video\component\log_functions;
if ( ! defined( 'ABSPATH' ) ) {
...
class wedesinLogSettings
{
    ...
    public function add_log($event, $message, $note="") {
        $log = new \wc_video\framework\log_functions\wedesinLog;
    ...
    public function the_log_excerpt_admin($file="") { 
        ...
        if($this->is_log_file_exists($file)) {
            $viewfile = $fileLog;
            if ($_GET['tab'] == $this->get_plugin_folder_name() ) { 
                include_once( TM_PATHWCVIDEO. 'framework/log/views/view-log.php' );
            }
        }
    }
}
 }
new \wc_video\component\log_functions\wedesinLogSettings;