<?php 
namespace wc_video\framework\forms_builder;
if ( ! defined( 'ABSPATH' ) ) {
  exit;
}
...
if( ! class_exists( 'formsBuilderWDS' ) )
{
	class formsBuilderWDS
	{
		...
		public function display_form($formID ) {
			...
			$this->wds_form = new \wc_video\framework\forms_core\WDS_Front_Form($field);
			echo $this->wds_form->get_form($formID );
			
		}
		public function save_form() {
			$inputs = filter_input_array(INPUT_POST);
            if (isset($inputs['form_id_wds']) && !empty($inputs['form_id_wds']) && isset($inputs['plugin_id_wds']) && $inputs['plugin_id_wds'] == TM_IDWCVIDEO ) {
		...
		public function get_fields_form($formID){
			$fields= [];
			$tpf = new \wc_video\admin\form_fields\thisPluginField;
    ...
    new \wc_video\framework\forms_builder\formsBuilderWDS;
